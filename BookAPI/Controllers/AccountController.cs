...
using BookAPI.IRepositories;
using BookAPI.Models;
using LoginRequest = BookAPI.ModelView.LoginRequest;
using RegisterRequest = BookAPI.ModelView.RegisterRequest;
...
public class AccountController:ControllerBase
{
    ...
    public async Task<IActionResult> Login(LoginRequest request)
    {
        ...
        var log =await _userManager.FindByNameAsync(request.UserName)
        ...
        var userName = log.UserName ?? log.Email ?? request.UserName;
        var Don = await _signInManager.PasswordSignInAsync(userName, request.Password, isPersistent: request.RememberMe, lockoutOnFailure: true);
        ...
        if (!Don.Succeeded)
        {
            if (Don.IsLockedOut)
            {
                ...
                if (log.LockoutEnd.HasValue && log.LockoutEnd.Value <= DateTimeOffset.UtcNow)
                {
                    ...
                    // إعادة المحاولة
                    Don = await _signInManager.PasswordSignInAsync(userName, request.Password, isPersistent: request.RememberMe, lockoutOnFailure: true);
                    if (Don.Succeeded)
                    {
                        return Ok(new
                        {
